...
-- =============================================
ALTER PROCEDURE [FIN].[SP_ProcessNewFAGLL03_v4]
AS
BEGIN
	...
	DELETE FROM FIN.[20_FAGLL03_Practice]

	INSERT INTO FIN.[20_FAGLL03_Practice]
		SELECT * FROM FIN.[19_FAGLL03 STAGING_2024_02_05]
	...
	BEGIN
		...
		DELETE FROM FIN.[21_FAGLL03_FINAL_Practice]
	...
	SET @fagll03count = (SELECT COUNT(*) FROM [FIN].[20_FAGLL03_Practice])
...
--This stored procedure replicates the Access Macro: 06_Generate File EERR
	...
	UPDATE [FIN].[20_FAGLL03_Practice] SET [Type] = 'CTB';

	-- 07_Incorpora SK21 ZA08Z01 NULL

	UPDATE [FIN].[20_FAGLL03_Practice] SET [20_FAGLL03_Practice].[Functional Area] = 'SK21'
	WHERE ((([FIN].[20_FAGLL03_Practice].[Functional Area]) Is Null) AND (([20_FAGLL03_Practice].[Profit Center])='ZA08Z01'));
	...
	INSERT INTO #Corridge_Profit
		...
		FROM [FIN].[20_FAGLL03_Practice]
		GROUP BY [20_FAGLL03_Practice].[Profit Center], [20_FAGLL03_Practice].[Cost Center], Left([Cost Center],7)
		HAVING ((([20_FAGLL03_Practice].[Cost Center])<>'' And ([20_FAGLL03_Practice].[Cost Center]) Is Not Null));
	...
	INSERT INTO FIN.[20_FAGLL03_Practice] 
		...
		[Material], [Order], [Partner Profit Ctr], [Plant], [Posting Period], [Reference], [User name],[Type])
			...
			[FIN].[20_FAGLL03_Practice].[Profit Center],
			[Quantity] * -1,
			[Text],[Base Unit of Measure], 
			[Company Code],[FIN].[20_FAGLL03_Practice].[Cost Center],
			...
			FROM [FIN].[20_FAGLL03_Practice] INNER JOIN #Corridge_Profit ON [FIN].[20_FAGLL03_Practice].
			[Cost Center] = #Corridge_Profit.[Cost Center] AND 
			[FIN].[20_FAGLL03_Practice].[Profit Center] = #Corridge_Profit.[Profit Center]
	...
	INSERT INTO #Corridge_Function_Area
		...
		FROM [FIN].[20_FAGLL03_Practice]
		GROUP BY [20_FAGLL03_Practice].[Cost Center], [20_FAGLL03_Practice].[Functional Area]
		HAVING ((([20_FAGLL03_Practice].[Cost Center])<>'' And ([20_FAGLL03_Practice].[Cost Center]) Is Not Null));
	...
	INSERT INTO FIN.[20_FAGLL03_Practice]( Account, Assignment, [Document Number], [Document Date], 
	...
	Material, [Order], [Partner Profit Ctr], Plant, [Posting period], Quantity, Reference, [User name], Type )
		...
		FIN.[20_FAGLL03_Practice].[Cost Center], [Cost Element], 
		[Document Number G/L], FIN.[20_FAGLL03_Practice].[Functional Area], 
		...
		FROM FIN.[20_FAGLL03_Practice] INNER JOIN #Corridge_Function_Area ON ([20_FAGLL03_Practice].[Functional Area] = #Corridge_Function_Area.[Functional Area]) 
		AND ([20_FAGLL03_Practice].[Cost Center] = #Corridge_Function_Area.[Cost Center])
	...
	[Material], [Order], [Partner Profit Ctr], [Plant], [Posting period], [Quantity], [Reference], [User name], Type )
		...
		[G/L Fiscal Year], [General ledger currency], 
		[General Ledger Item], [Invoice reference], 
		[Material], [Order], FIN.[20_FAGLL03_Practice].[Partner Profit Ctr], 
		[Plant], [Posting period], 
		[Quantity] *-1, [Reference], 
		[User name], 'GST' AS Type
		FROM FIN.[20_FAGLL03_Practice] INNER JOIN #Corridge_Function_Area ON ([20_FAGLL03_Practice].[Functional Area] = #Corridge_Function_Area.[Functional Area]) AND ([20_FAGLL03_Practice].[Cost Center] = #Corridge_Function_Area.[Cost Center])
    ...
    -- DELETE FROM FIN.[21_FAGLL03 FINAL] where FIN.[21_FAGLL03 FINAL].[G/L Fiscal Year] = FIN.[20_FAGLL03_Practice].[G/L Fiscal Year]
	-- and FIN.[21_FAGLL03 FINAL].[Posting period] =  FIN.[20_FAGLL03_Practice].[Posting period]

	-- 30_Crea TB21 FAGLL03 Final

	INSERT INTO FIN.[21_FAGLL03_FINAL_Practice] ([Version],[G/L Fiscal Year],[Posting period]
    ...
    ,[User name],[Status],[R2A],[R3A],[R4A])
		...
		MCP.[Responsible4A]
		FROM ((((FIN.[20_FAGLL03_Practice] FAG 
	...
	SET @fagll03finalcount = (SELECT COUNT(*) FROM [FIN].[21_FAGLL03_FINAL_Practice])

	PRINT 'FAGLL03 FINAL Count : '+CAST(@fagll03finalcount AS varchar(max))

	-- 31_Corrige Function Area NULL

	UPDATE FIN.[21_FAGLL03_FINAL_Practice] 
	...
	UPDATE FIN.[21_FAGLL03_FINAL_Practice]
		...
		FROM FIN.[21_FAGLL03_FINAL_Practice] FAG
	...
	UPDATE FIN.[21_FAGLL03_FINAL_Practice] SET Type = 'GST'
		WHERE (((Type)='GS2'));
